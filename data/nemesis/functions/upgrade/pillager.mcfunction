...
execute unless entity @e[tag=nem_uninstall] run tellraw @s {"text":"The Pillager has increased the raid","italic":true,"color":"dark_red"}
advancement revoke @s only nemesis:pillager
tag @e[tag=nemesis] remove nemesis
execute unless entity @e[tag=nem_uninstall] run scoreboard players add @s[scores={nem_pillager=..4}] nem_pillager 1
