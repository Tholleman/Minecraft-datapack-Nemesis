...
execute if score rng nem_num matches 3 at @e[tag=nemesis] run summon minecraft:skeleton ~ ~ ~ {CustomName:"\"Wither Skeleton\"",Tags:["nem_skeleton_wither_skeleton"],PersistenceRequired:1b,Health:30,Attributes:[{Name:"generic.maxHealth",Base:30},{Name:"generic.followRange",Base:64}],HandItems:[{id:"minecraft:stone_sword",Count:1},{}],ArmorItems:[{},{},{},{id:"minecraft:wither_skeleton_skull",Count:1}]}

execute if entity @e[tag=nemesis] run tellraw @s {"text":"The Skeleton has grown more powerful","italic":true,"color":"dark_red"}
